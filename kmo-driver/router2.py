...
APP_FILE = Path(__file__).resolve().parent / 'rastreamento-psicologico' / 'index.html'

class Router(BaseHTTPRequestHandler):
    protocol_version = 'HTTP/1.1'

    def log_message(self, fmt, *args):
        print('[ROUTER2]', fmt % args, flush=True)
    ...
    def psychological_screening(self):
        try:
            self.send_bytes(200, APP_FILE.read_bytes())
...
if __name__ == '__main__':
    print(f'[ROUTER2] Porta pública {PUBLIC_PORT}; gateway interno {GATEWAY_PORT}', flush=True)
